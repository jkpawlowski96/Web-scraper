...
def scrap_website(address):
	...
	row = {'Address': address, 'Text': web_text, 'Images': web_images_b, 'Images_links': web_images_l}
	#  add resource to database
	if data_add_resource(row) is True:
		return True
	else:
		return False
...
class Scrapper(threading.Thread):
	...
	def run(self):
		...
		while():
			try:
				address = self.job_queue.pop(0)  # Take address to do from the stack
			except:
				# no tasks
			...
			try:
				...
				if results is True:
					address_working(address, value=False)  # Job is not working anymore. address is already in database
					continue
				else:
